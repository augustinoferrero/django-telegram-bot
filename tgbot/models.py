...
class Zakladka(CreateUpdateTracker):
    ...
    def __str__(self):
        return f"Закладка: {self.product} Фасовка: {self.fasovka} Товар: {self.product} Город: {self.city} Район: {self.district} Тип клада: {self.klad_type}"
...
class Order(CreateUpdateTracker):
    ...
    price = models.FloatField(verbose_name="Цена", null=True, blank=True) # btc
